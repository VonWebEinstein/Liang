...
% x 可以是数值型数组、字符串、字符型数组、字符串元胞数组和名义尺度数组。
%
% result = HistRate(x)，返回矩阵或元胞数组result，它是多行4 列的矩阵或元胞数组，
% 四列分别对应取值、频数、频率、累积频率。当x 是数值型数组时，result 为矩阵；当x
% 是字符串、字符型数组、字符串元胞数组和名义尺度数组时，result 为元胞数组。
%
% 用户还可参考tabulate 函数，该函数比tabulate 函数的效率高。
%
